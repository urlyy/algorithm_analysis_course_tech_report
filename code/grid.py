...
pygame.display.set_caption("多小球碰撞模拟 grid")
    ...
    for dx in range(-1, 2):
        for dy in range(-1, 2):
            if (grid_x + dx, grid_y + dy) in grid:
                nearby_balls.extend(grid[(grid_x + dx, grid_y + dy)])
...
while running:
    ...
    for ball in balls:
        ...
        nearby_balls = get_nearby_balls(ball)
        for nearby_ball in nearby_balls:
            if ball != nearby_ball:
                ball.collide(nearby_ball)
