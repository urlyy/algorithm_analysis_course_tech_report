...
pygame.display.set_caption("多小球碰撞模拟 sort_and_sweep")
...
while running:
    ...
    # 按照x坐标进行排序（使用左边界x - radius）
    balls.sort(key=lambda ball: ball.x - ball.radius)
    ...
    # 使用Sort and Sweep方法只检查相邻的小球碰撞
    for i in range(len(balls)):
        for j in range(i + 1, len(balls)):
            # 如果j小球的左边界已经超过了i小球的右边界，后续的小球就不可能发生碰撞
            if balls[j].x - balls[j].radius > balls[i].x + balls[i].radius:
                break
